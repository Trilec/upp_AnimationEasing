...
// -----------------------------------------------------------------------------
//  PURPOSE
//  ‑ Base animation layer for the U++ framework (V1).
//  ‑ Offers deterministic, zero-heap-leak, single-timer animation for
//    Rect, Point, Size, Color, double, int and any user type via AnimateValue<T>.
//  ‑ Designed to be header-only friendly and extensible for future tween
//    curves, pause/resume, chaining, etc.
// -----------------------------------------------------------------------------
...
namespace Upp {
	...
	public:
	    struct State {
			...
			void Cancel();
			explicit State(Ctrl& c) : owner(&c) {}
	    };
	private:
	    One<State> s_owner;
	    State*     s_handle = nullptr;
	public:
	    Animation() = default;
	    explicit Animation(Ctrl& c) { s_owner = new State(c); s_handle = s_owner.Get(); }
	    ~Animation() { Stop(); }
	    Animation(Animation&& o)            { *this = pick(o); }
	    Animation& operator=(Animation&& o) { Stop(); s_owner = pick(o.s_owner); s_handle = o.s_handle; o.s_handle = nullptr; return *this; }
	    ...
	    Animation& Time(int ms)            { if(s_handle) s_handle->time_ms = ms; return *this; }
	    Animation& Ease(Easing::Fn fn)     { if(s_handle) s_handle->ease   = fn; return *this; }
	    Animation& Count(int cnt)          { if(s_handle) s_handle->count  = cnt; return *this; }
	    Animation& Yoyo(int cnt = -1)      { if(s_handle) { s_handle->yoyo = true; s_handle->count = cnt < 0 ? -1 : 2*cnt; } return *this; }
	    Animation& operator()(Function<bool(double)> f) { if(s_handle) s_handle->tick = pick(f); return *this; }
	...
	// ------------------------------------------------------------------
	//  Generic helper + one-liner wrappers
	// ------------------------------------------------------------------
	template <typename T>
	inline Animation AnimateValue(Ctrl& c,
	                              Gate<T> setter,
	                              const T& from,
	                              const T& to,
	                              int ms,
	...
	inline Animation AnimateDouble (Ctrl& c, Gate<double> g,double f,double t,int ms,Easing::Fn e=Easing::InOutQuad,AnimMode m=Once){return AnimateValue(c,g,f,t,ms,e,m);}
	inline Animation AnimateInteger(Ctrl& c, Gate<int>    g,int    f,int    t,int ms,Easing::Fn e=Easing::InOutQuad,AnimMode m=Once){return AnimateValue(c,g,f,t,ms,e,m);}
	inline Animation AnimateColor  (Ctrl& c, Gate<Color>  g,Color  f,Color  t,int ms,Easing::Fn e=Easing::InOutQuad,AnimMode m=Once){return AnimateValue(c,g,f,t,ms,e,m);}
	inline Animation AnimatePoint  (Ctrl& c, Gate<Point>  g,Point  f,Point  t,int ms,Easing::Fn e=Easing::InOutQuad,AnimMode m=Once){return AnimateValue(c,g,f,t,ms,e,m);}
	inline Animation AnimateSize   (Ctrl& c, Gate<Size>   g,Size   f,Size   t,int ms,Easing::Fn e=Easing::InOutQuad,AnimMode m=Once){return AnimateValue(c,g,f,t,ms,e,m);}
	inline Animation AnimateRect   (Ctrl& c, Gate<Rect>   g,Rect   f,Rect   t,int ms,Easing::Fn e=Easing::InOutQuad,AnimMode m=Once){return AnimateValue(c,g,f,t,ms,e,m);}
	inline Animation AnimateAlpha  (Ctrl& c, double from, double to, int ms, Easing::Fn e=Easing::InOutQuad, AnimMode m=Once)
	{
